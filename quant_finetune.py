...
def finetune(
    ...
    gaussians = GaussianModel(dataset.sh_degree, quantization=True)
    scene = Scene(
        dataset,
        gaussians,
        load_iteration = -1, 
        load_vq=args.load_vq,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--start_iter", nargs="+", type=int, default=30_000
    )
    parser.add_argument(
        "--test_iterations", nargs="+", type=int, default=[30_001, 31_001, 35_400]
    )
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[35_400])
