...
get_available_gpu() {
  ...
  local gpu_id=2 # Specify the GPU ID to check
  nvidia-smi --query-gpu=index,memory.used --format=csv,noheader,nounits -i $gpu_id | awk -v threshold="$mem_threshold" -F', ' '
      ...
      while true; do
        ...
        if [[ -n $gpu_id ]]; then
          ...
          CUDA_VISIBLE_DEVICES=$gpu_id python train.py \
            ...
            -m "output_123/${arg}_${prune_percent}" \
            --eval \
            --port $port \
            --iteration 30000
